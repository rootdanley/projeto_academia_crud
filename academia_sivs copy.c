...
int obter_max_alunos_turma(int turma_id)
{
    ...
    while (fgets(linha, sizeof(linha), arquivo))
    {
        int inscricao_id, aluno_id, turma_id_csv;
        sscanf(linha, "%d,%d,%d", &inscricao_id, &aluno_id, &turma_id_csv);
        if (turma_id_csv == turma_id)
        {
            quantidade_alunos++;
        }
    }
...
void cadastrar_inscricao()
{
  struct tb_inscricoes inscricao;
    ...
    int turma_limite_max = 0;

    while (fgets(linha, sizeof(linha), arquivo))
    {
        int turma_id;
        sscanf(linha, "%d,%*[^,],%*[^,],%d", &turma_id, &turma_limite_max);
        if (turma_id == inscricao.inscricao_id_turma)
        {
            turmaEncontrada = 1;
            break;
        }
    ...
    int numAlunosInscritos = obter_quantidade_alunos_inscritos(inscricao.inscricao_id_turma);
    int numMaxInscricoes = turma_limite_max;
    int numVagasDisponiveis = numMaxInscricoes - numAlunosInscritos;
